-- ══════════════════════════════════════════════════════════════════════════
-- Migration: Fix payment_method enum cast in complete_ride_transactional
-- Date: 2027-06-04
-- Description: Replaces v_ride.payment_method IN ('wallet', 'nas_wallet')
--              with v_effective_payment_method IN ('wallet', 'nas_wallet')
--              and adds explicit casts back to public.payment_method.
-- ══════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Replaced v_ride.payment_method with v_effective_payment_method (which is TEXT)
  -- to avoid casting exception in case "wallet" is not a valid enum value in database
  IF v_effective_payment_method IN ('wallet', 'nas_wallet') THEN
  ...
  ELSE
    ...
    UPDATE rides
    SET payment_method = v_effective_payment_method::public.payment_method
    WHERE id = p_ride_id;
  ...
  SET
    ...
    payment_method = v_effective_payment_method::public.payment_method,
...
'Atomic ride completion with server-side wait time calculations, 30-minute cap, financial updates, and payment enum fix.';
