-- ══════════════════════════════════════════════════════════════
-- 💳 إكمال الرحلة الذري والمعالجة المالية (complete_ride_transactional)
-- ══════════════════════════════════════════════════════════════
-- الوصف: دالة RPC تقوم بإكمال الرحلة والمعالجة المالية في 
--       صفقة (Transaction) واحدة ذرية بقاعدة البيانات.
-- ══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  v_waiting_mins := p_waiting_minutes;
  IF v_ride.driver_arrival_time IS NOT NULL AND v_ride.started_at IS NOT NULL THEN
    DECLARE
      v_server_calc INTEGER;
    BEGIN
      v_server_calc := EXTRACT(EPOCH FROM (v_ride.started_at - v_ride.driver_arrival_time))::INTEGER / 60;
      IF v_server_calc > 0 THEN
        v_waiting_mins := GREATEST(v_server_calc, COALESCE(v_waiting_mins, 0));
      END IF;
    END;
  END IF;
  v_waiting_mins := COALESCE(v_waiting_mins, 0);
...
GRANT EXECUTE ON FUNCTION public.complete_ride_transactional(UUID, UUID, DOUBLE PRECISION, INTEGER) TO authenticated;
...
'إكمال الرحلة الذري: تعديل أجرة + خصم محفظة راكب + عمولة سائق + تسجيل أرباح — كل شيء في transaction واحدة';
