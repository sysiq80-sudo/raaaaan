...
function isVehicleTypeCompatible(driverType: string, rideType: string): boolean {
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: drivers, error: driversError } = await supabase
      .from('drivers')
      .select('id, user_id, full_name, vehicle_type, current_location, rating, total_rides, max_pickup_radius')
      .eq('is_online', true)
    ...
    const driversWithDistance = drivers
      .filter(driver => {
        ...
        if (!isVehicleTypeCompatible(driver.vehicle_type, ride.vehicle_type)) return false;
        // Skip already notified drivers
        if (alreadyNotified.has(driver.id)) return false;
        return true;
      })
      .map(driver => {
        const driverLoc = driver.current_location as { lat: number; lng: number };
        ...
        const vehicleMatchBonus = driver.vehicle_type === ride.vehicle_type ? 20 : 0;
        // Penalty if distance exceeds driver's preferred radius
        const radiusPenalty = distance > maxRadius ? -30 : 0;
        
        const priorityScore = distanceScore + ratingScore + experienceScore + vehicleMatchBonus + radiusPenalty;
      ...
      .filter(driver => driver.distance_km <= Math.max(driver.max_radius, 15))
    ...
    console.log('🏆 أفضل السائقين:', topDrivers.map(d => ({
      name: d.full_name,
    ...
    const allNotifiedDrivers = [...Array.from(alreadyNotified), ...topDrivers.map(d => d.id)];
    await supabase
      .from('rides')
      .update({ 
      ...
      try {
        ...
        const { error: notifError } = await supabase.functions.invoke('send-push-notification', {
          body: {
            ...
            title: '🚗 طلب رحلة جديد!',
            body: `على بعد ${driver.distance_km} كم • ${ride.estimated_fare?.toLocaleString() || 0} د.ع`,
            data: {
        ...
        if (notifError) {
          console.error(`❌ فشل إرسال الإشعار للسائق ${driver.full_name}:`, notifError);
          return { success: false, error: notifError };
        }

        console.log(`✅ إشعار للسائق ${driver.full_name} (${index + 1}/${topDrivers.length})`);
    ...
    console.log(`📨 تم إرسال ${successCount}/${topDrivers.length} إشعار (${processingTime}ms)`);
