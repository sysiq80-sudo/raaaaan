...
const AdminLayout = ({ children, title, subtitle, actions }: AdminLayoutProps) => {
  ...
  return (
    <div className="min-h-screen bg-background flex" dir="rtl">
        ...
        <ScrollArea className="flex-1 py-2">
          <nav className={cn("space-y-1", collapsed ? "px-2" : "px-4")}>
            ...
            {collapsed ? (
              ...
              <Collapsible open={pricingOpen} onOpenChange={setPricingOpen} className="mt-2">
                <CollapsibleTrigger className={cn(
                  "flex items-center w-full rounded-lg transition-all duration-200 gap-3 px-4 py-3",
                  advancedPricingItems.some(item => location.pathname === item.href)
                    ? 'bg-sidebar-accent text-sidebar-foreground'
                    : 'text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-foreground'
                )}>
                  <DollarSign className="w-5 h-5 flex-shrink-0" />
                  <span className="flex-1 text-right whitespace-nowrap">التسعير المتقدم</span>
      ...
      {/* Main Content */}
      <main className={cn("flex-1 transition-all duration-300 ease-in-out", mainMargin)}>
        {/* Top Header Bar */}
