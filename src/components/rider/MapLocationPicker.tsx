...
const MapLocationPicker: React.FC<MapLocationPickerProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchToken = async () => {
      try {
        const response = await fetch('https://wgolkcztdrwdphwjvqxt.supabase.co/functions/v1/mapbox-proxy?action=token', {
          headers: {
  ...
  const checkServiceArea = useCallback(async (lat: number, lng: number) => {
    try {
      ...
      const response = await fetch(`https://wgolkcztdrwdphwjvqxt.supabase.co/functions/v1/check-service-area?lat=${lat}&lng=${lng}`);
      const data = await response.json();
  ...
  const reverseGeocode = useCallback(async (lat: number, lng: number) => {
    try {
      const response = await fetch(`https://wgolkcztdrwdphwjvqxt.supabase.co/functions/v1/mapbox-proxy?action=reverse-geocode&lat=${lat}&lng=${lng}`, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
  ...
  useEffect(() => {
    ...
    const initialCenter = initialLocation ? [initialLocation.lng, initialLocation.lat] as [number, number] : userLocation ? [userLocation.lng, userLocation.lat] as [number, number] : ramadiCenter;
  ...
  const glowColor = isPickup ? '0 0 40px rgba(34, 197, 94, 0.8), 0 0 80px rgba(34, 197, 94, 0.4)' : '0 0 50px rgba(59, 130, 246, 0.8), 0 0 100px rgba(59, 130, 246, 0.4)';
  const gradientColor = isPickup ? 'linear-gradient(to bottom, #22c55e, transparent)' : 'linear-gradient(to bottom, #3b82f6, transparent)';
  ...
  return <div className="fixed inset-0 z-50 bg-background flex flex-col">
      ...
      <div className="flex-1 relative">
        ...
        {!isDragging && centerAddress && <div className="absolute top-24 left-1/2 transform -translate-x-1/2 z-10 pointer-events-none">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
          <div className="flex flex-col items-center">
            ...
            <div className="relative">
              {/* Pin head */}
              <div className={`w-8 h-8 rounded-full flex items-center justify-center shadow-xl border-2 border-white ${isPickup ? 'bg-green-500' : 'bg-blue-500'}`}>
                {isPickup ? <Target className="w-4 h-4 text-white" /> : <MapPin className="w-4 h-4 text-white" />}
              </div>

              {/* Pin needle pointing down to exact location */}
              <div className={`w-0.5 h-6 mx-auto ${isPickup ? 'bg-green-500' : 'bg-blue-500'}`} style={{
        ...
        {/* Loading overlay */}
        {isLoading && <div className="absolute inset-0 bg-card/90 backdrop-blur-md flex items-center justify-center z-30">
            <div className="text-center">
              <div className="w-14 h-14 rounded-full mx-auto mb-4" style={{
        ...
        {userLocation && <button onClick={centerOnUser} className="absolute bottom-5 left-4 w-14 h-14 bg-card/95 backdrop-blur-md rounded-2xl border border-border/50 shadow-xl flex items-center justify-center hover:bg-accent transition-all duration-200 active:scale-95 z-40 group" aria-label="تحديد موقعي الحالي">
      ...
      <div className="bg-card/95 backdrop-blur-md border-t border-border/50 shadow-2xl z-30">
        <div className="p-3 sm:p-4">
          ...
          {serviceAreaStatus && !serviceAreaStatus.in_service && <div className="flex items-center gap-3 p-3 mb-3 rounded-xl bg-gradient-to-r from-amber-500/10 to-orange-500/10 border border-amber-500/30 shadow-lg">
              ...
              <div className="flex-1">
                ...
                {serviceAreaStatus.nearest_region && <p className="text-amber-700 text-xs">
          ...
          <div className="flex items-start gap-3 mb-4 p-3 rounded-xl border bg-card/50">
            <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center shrink-0 border ${isPickup ? 'bg-green-500/20 border-green-400 shadow-lg shadow-green-500/20' : 'bg-blue-500/20 border-blue-400 shadow-lg shadow-blue-500/20'}`} style={{
            boxShadow: isPickup ? '0 0 20px rgba(34, 197, 94, 0.3), inset 0 2px 4px rgba(255, 255, 255, 0.1)' : '0 0 20px rgba(59, 130, 246, 0.3), inset 0 2px 4px rgba(255, 255, 255, 0.1)'
          }}>
              {isPickup ? <Target className="w-6 h-6 sm:w-7 sm:h-7 text-green-600 drop-shadow-sm" /> : <MapPin className="w-6 h-6 sm:w-7 sm:h-7 text-blue-600 drop-shadow-sm" />}
            </div>
            <div className="flex-1 min-w-0">
              ...
              {centerAddress && <p className="text-xs text-muted-foreground mt-1">
          ...
          <div className="mb-3">
            <LocationSearchInput placeholder={isPickup ? 'اكتب لتحديد موقع الانطلاق...' : 'اكتب لتحديد الوجهة...'} value={searchQuery} onChange={setSearchQuery} onLocationSelect={handleSearchSelect} type={isPickup ? 'pickup' : 'dropoff'} userLocation={userLocation} className="w-full" />
          </div>

          {/* Saved Places Section - Horizontal Scrolling (only show in dropoff mode) */}
          {!isPickup && savedPlaces.length > 0 && <div className="mb-4">
              <p className="text-xs font-semibold text-muted-foreground mb-3 px-1 flex items-center gap-2">
                <Star className="w-3 h-3 text-amber-600" />
                أماكني المحفوظة
              </p>
              <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
                {loadingSavedPlaces ? <div className="flex items-center justify-center w-full py-4">
                    <Loader2 className="w-4 h-4 animate-spin text-muted-foreground" />
                  </div> : savedPlaces.map(place => <button key={place.id} onClick={() => handleSavedPlaceSelect(place)} className="flex flex-col items-center justify-center flex-shrink-0 w-24 h-28 p-2 rounded-xl border border-border/50 hover:border-amber-500 hover:bg-amber-500/10 hover:shadow-md transition-all duration-200 group overflow-hidden bg-card/50">
          ...
          <Button onClick={handleConfirm} disabled={isCheckingService || !centerAddress} className={`w-full h-12 sm:h-14 text-base sm:text-lg font-bold rounded-xl transition-all duration-300 transform hover:scale-[1.01] active:scale-[0.99] ${isPickup ? 'bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 shadow-lg shadow-green-500/30' : 'bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 shadow-lg shadow-blue-500/30'} disabled:opacity-50 disabled:cursor-not-allowed`} style={{
          boxShadow: isPickup ? '0 4px 20px rgba(34, 197, 94, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.2)' : '0 4px 20px rgba(59, 130, 246, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.2)'
        }}>
            {isCheckingService ? <div className="flex items-center gap-2">
