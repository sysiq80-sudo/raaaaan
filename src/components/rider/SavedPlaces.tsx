...
import { Home, Briefcase, MapPin, Plus, X, Loader2, Check, Star, Heart, GraduationCap, Dumbbell, Utensils, Cross, ShoppingBag, Building2, Edit3 } from 'lucide-react';
...
const SavedPlaces: React.FC<SavedPlacesProps> = ({
  ...
  const handleDeletePlace = async (id: string, e: React.MouseEvent) => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "حدث خطأ في حذف الموقع";
      toast({
        title: "خطأ",
  ...
  if (compact) {
    ...
    return <div className="flex gap-2">
        {homePlace && <button onClick={() => onSelect(homePlace)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-blue-500/10 hover:bg-blue-500/20 text-blue-600 transition-all">
            <Home className="w-4 h-4" />
            <span className="text-sm font-medium">المنزل</span>
          </button>}
        {workPlace && <button onClick={() => onSelect(workPlace)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 transition-all">
  ...
  return <div className="space-y-2">
      ...
      {places.length === 0 && showAddButton && currentLocation && <div className="flex gap-2">
      ...
      {places.length > 0 && <>
          ...
          {places.map(place => <div key={place.id} className="flex items-center gap-3 p-3 rounded-xl bg-secondary/50 hover:bg-secondary transition-all group animate-in fade-in-50 duration-300">
              <button onClick={() => onSelect(place)} className="flex-1 flex items-center gap-3 text-right">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={e => handleEditPlace(place, e)} className="p-2 rounded-lg hover:bg-primary/10 text-primary transition-all">
                  <Edit3 className="w-4 h-4" />
                </button>
                <button onClick={e => handleDeletePlace(place.id, e)} className="p-2 rounded-lg hover:bg-destructive/10 text-destructive transition-all">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>)}
      ...
      {showAddButton && currentLocation && <Button variant="ghost" size="sm" onClick={() => setShowAddDialog(true)} className="w-full gap-2 text-muted-foreground hover:text-primary">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              {PRESET_LABELS.map(preset => <button key={preset.label} onClick={() => {
              setSelectedLabel(preset.label);
              setSelectedIcon(preset.icon);
            }} className={`p-3 rounded-xl text-center transition-all ${selectedLabel === preset.label ? 'bg-primary text-primary-foreground scale-105' : 'bg-secondary hover:bg-secondary/80'}`}>
            ...
            <div>
              <button onClick={() => setShowIconPicker(!showIconPicker)} className="text-xs text-primary hover:underline">
                {showIconPicker ? 'إخفاء الأيقونات' : 'اختيار أيقونة مختلفة'}
              </button>
              
              {showIconPicker && <div className="grid grid-cols-6 gap-2 mt-3 p-3 rounded-xl bg-secondary/30 animate-in fade-in-50 duration-200">
                  {EXTENDED_ICONS.map(iconOption => <button key={iconOption.label} onClick={() => {
                setSelectedIcon(iconOption.icon);
                setSelectedLabel(iconOption.label);
              }} className={`p-2 rounded-lg text-center transition-all ${selectedIcon === iconOption.icon ? 'bg-primary text-primary-foreground' : 'hover:bg-secondary'}`} title={iconOption.name}>
            ...
            <div>
              ...
              <Input value={customName} onChange={e => setCustomName(e.target.value)} placeholder={PRESET_LABELS.find(p => p.label === selectedLabel)?.name || "مثال: بيت جدتي"} />
            </div>

            {/* Location preview */}
            {currentLocation && <div className="p-3 rounded-xl bg-secondary/50">
            ...
            <Button onClick={handleSavePlace} disabled={saving || !currentLocation} className="w-full">
