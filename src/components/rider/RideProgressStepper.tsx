...
const RideProgressStepper: React.FC<RideProgressStepperProps> = ({
  ...
  return <div className="w-full bg-gradient-to-b from-card via-card to-transparent py-0 px-0">
      ...
      <div className="relative">
        ...
        <motion.div className="absolute top-6 right-6 h-2 rounded-full overflow-hidden" initial={{
        width: 0
      }} animate={{
          ...
          return <motion.div key={step.key} initial={{
            ...
            width: `${100 / steps.length}%`
                ...
                <div className="relative">
                  ...
                  {isCurrent && <>
                      <motion.div className="absolute -inset-3 rounded-full bg-primary/20" animate={{
                  scale: [1, 1.2, 1],
                  opacity: [0.5, 0.2, 0.5]
                }} transition={{
                  duration: 2,
                ...
                scale: 1.05
                    ...
                    {/* Icon */}
                    {isCurrent && status !== 'completed' ? <Loader2 className="w-5 h-5 animate-spin relative z-10" /> : isCompleted || isCurrent && status === 'completed' ? <CheckCircle className="w-5 h-5 relative z-10" /> : <Icon className="w-5 h-5 relative z-10" />}
                  </motion.div>
                </div>
              ...
              delay: 0.3 + index * 0.1
            }} className={cn("text-[10px] mt-2.5 text-center leading-tight font-medium transition-all duration-500", isCompleted && "text-emerald-600 dark:text-emerald-400 font-semibold", isCurrent && "text-primary font-bold", isPending && "text-muted-foreground/50")}>
                  {step.label}
                </motion.span>

                {/* Active Indicator Dot */}
                {isCurrent && <motion.div className="absolute -bottom-1 w-2 h-2 rounded-full bg-primary" animate={{
              y: [0, -3, 0]
            }} transition={{
              duration: 1,
              repeat: Infinity
      ...
      <div className="mt-5">
        <AnimatePresence mode="wait">
          ...
          ease: "easeInOut"
            ...
            {status === 'completed' && <Sparkles className="w-4 h-4 text-amber-500 animate-pulse" />}
            <p className={cn("text-sm font-semibold text-center", status === 'completed' ? "bg-gradient-to-r from-emerald-400 via-primary to-amber-500 bg-clip-text text-transparent" : messages[messageIndex]?.includes('✅') ? "text-emerald-500" : messages[messageIndex]?.includes('⏱️') ? "text-primary" : "text-foreground/80")}>
