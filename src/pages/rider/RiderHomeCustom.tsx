...
const RiderHomeCustom: React.FC = () => {
  ...
  return <div className="h-screen w-full relative overflow-hidden flex flex-col">
      ...
      <div className="h-1/2 w-full relative z-0">
        ...
        <button onClick={getCurrentLocation} disabled={isLocating} className="absolute top-1/2 left-1/2 z-50 w-14 h-14 bg-card/95 backdrop-blur-md rounded-2xl border border-border/50 shadow-2xl flex items-center justify-center hover:bg-accent transition-all duration-200 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed" style={{
        transform: "translate(-140%, -50%)"
      }} aria-label="تحديد موقعي الحالي">
          {isLocating ? <div className="animate-spin rounded-full h-6 w-6 border-3 border-primary border-t-transparent" /> : <Navigation className="w-6 h-6 text-primary" />}
        </button>
      </div>
