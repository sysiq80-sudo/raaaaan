...
import { CustomizationProvider, useCustomization } from "@/components/rider/CustomizationPanel";
...
const RiderHomeCustom: React.FC = () => {
  ...
  // Location State - استرجاع البيانات المحفوظة من sessionStorage
  const [pickup, setPickup] = useState(() => {
    return sessionStorage.getItem("raan_pickup_address") || "";
  });
  const [dropoff, setDropoff] = useState(() => {
    return sessionStorage.getItem("raan_dropoff_address") || "";
  });
  const [pickupCoords, setPickupCoords] = useState<{
    lat: number;
    lng: number;
  } | null>(() => {
    const saved = sessionStorage.getItem("raan_pickup_coords");
  ...
  const [dropoffCoords, setDropoffCoords] = useState<{
    ...
    const saved = sessionStorage.getItem("raan_dropoff_coords");
    return saved ? JSON.parse(saved) : null;
  });
  ...
  // دائماً نبدأ بفتح MapPicker إذا لم يكن هناك موقع انطلاق محفوظ
  const [showMapPicker, setShowMapPicker] = useState(() => {
    const savedPickup = sessionStorage.getItem("raan_pickup_coords");
    return !savedPickup;
  });
  ...
  // حفظ البيانات في sessionStorage عند تغييرها
  useEffect(() => {
    if (pickupCoords) {
      sessionStorage.setItem("raan_pickup_coords", JSON.stringify(pickupCoords));
    }
  }, [pickupCoords]);
  useEffect(() => {
    if (pickup) {
      sessionStorage.setItem("raan_pickup_address", pickup);
    }
  }, [pickup]);
  useEffect(() => {
    if (dropoffCoords) {
      sessionStorage.setItem("raan_dropoff_coords", JSON.stringify(dropoffCoords));
    }
  }, [dropoffCoords]);
  useEffect(() => {
    if (dropoff) {
      sessionStorage.setItem("raan_dropoff_address", dropoff);
    }
  }, [dropoff]);
  ...
  return <div className="h-screen w-full relative overflow-hidden flex flex-col">
      ...
      <div className="h-1/2 w-full bg-background relative z-10 overflow-y-auto pt-4">
        ...
        {isLocating && !showSearchOverlay && !showBookingPanel && <div className="flex flex-col items-center justify-center h-full space-y-4">
              ...
              <div className="bg-gradient-to-t from-card via-card/98 to-card/90 backdrop-blur-lg rounded-2xl shadow-lg p-4">
                ...
                <div className="mb-4">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    // مسح بيانات الوجهة فقط من sessionStorage
                    sessionStorage.removeItem("raan_dropoff_coords");
                    sessionStorage.removeItem("raan_dropoff_address");
                ...
                {/* Payment Method */}
                <div className="border-t border-border/20 pt-4">
                  <div className="w-full flex items-center justify-between cursor-pointer" onClick={() => setShowPaymentSheet(true)}>
      ...
      {/* Quick Gestures */}
      <QuickGestures onSwipeUp={handleQuickBook} onDoubleTap={handleEmergency} onLongPress={() => {}} enabled={settings.gestures.swipe_book || settings.gestures.double_tap_emergency || settings.gestures.long_press_menu} />
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-medium">
                {selectedVehicle === "economy" ? "اقتصادي" : selectedVehicle === "comfort" ? "مريح" : selectedVehicle === "premium" ? "فاخر" : "نسائي"}
              </span>
            </div>
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">طريقة الدفع:</span>
              <span className="font-medium">
                {paymentMethod === "cash" ? "نقداً" : paymentMethod === "wallet" ? "المحفظة" : paymentMethod === "card" ? "بطاقة" : paymentMethod}
              </span>
            </div>

            {/* Drivers Available */}
            {nearbyDriversCount > 0 && <div className="flex items-center gap-2 text-sm text-green-600 bg-green-50 dark:bg-green-900/20 rounded-lg p-2">
